...
namespace QuickHost
{
    ...
    public class ServiceStackHost : AppHostHttpListenerBase
    {
        ...
        public static ServiceStackHost CreateFrom(string serviceName, object quickHostableClass)
        {
            ...
            return new ServiceStackHost(serviceName, MapToServiceStack(serviceName, quickHostableClass, out routeMapping), routeMapping);   
        }
        ...
        private static Type[] MapToServiceStack(string serviceName, object quickHostableClass, out Dictionary<string, Type> name_map)
        {
            ...
            Type t = quickHostableClass.GetType();
            ...
            if (methods_to_map.Count != 0)
            {
                ...
                foreach (MethodInfo mi in methods_to_map.Keys)
                {
                    ...
                    TypeBuilder servicebuilder = mod_builder.DefineType("InterfaceHost.Dynamic." + /*quickHostableClass.HostedShortName +*/ "." + method + "Server", type_attributes,
                    typeof(ServiceStack.ServiceInterface.Service));
                    TypeBuilder inputbuilder = mod_builder.DefineType("InterfaceHost.Dynamic." + /*quickHostableClass.HostedShortName +*/ "." + method, type_attributes, null);
                    TypeBuilder outputbuilder = mod_builder.DefineType("InterfaceHost.Dynamic." + /*quickHostableClass.HostedShortName +*/ "." + method + "Response", type_attributes, null);
                    ...
                    MethodInfo mapping_func = typeof(QuickHostableClassMappings).GetMethod("GetHostedClass");
                    ...
                    il_gen.Emit(OpCodes.Ldstr, serviceName);
